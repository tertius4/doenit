<script>
  ...
  const is_form_page = $derived(["/(main)/create", "/(main)/[item_id]"].includes(page_id));
  const is_home = $derived(page_id === "/(main)");
  ...
  const onclick = $derived(is_form_page ? null : () => goto(page_id === "/(main)" ? "/create" : "/"));
</script>
...
{:else if icon_name}
  <button
    {type}
    {form}
    class="flex justify-center items-center aspect-square rounded-full h-15 w-15 p-3"
    class:bg-primary={is_home}
    class:bg-card={!is_home}
    {onclick}
  ...
  >
    ...
    {:else}
      <Icon name={icon_name} class="text-2xl {is_home ? 'text-white' : ''}" />
    {/if}
  </button>
{/if}
