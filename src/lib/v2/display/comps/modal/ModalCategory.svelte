<script>
   ...
   * @prop {DB.Category} [category] - The category to edit. If not provided, a new category will be created.
   * @prop {(name: string) => *} [oncreate] - Callback function to call when a category is created.
  ...
  const { oncreate, onclose, category, ...rest } = props;
  ...
  async function addCategory() {
    const result = await Api.cats.save({ name });
    if (!result.ok) return (error_message = result.error);

    name = "";
    open = false;

    if (oncreate) oncreate(result.value.id);
  }
...
<Modal bind:is_open={open} onclose={handleClose} onsubmit={addCategory} class="space-y-4" {...rest}>
  ...
  <button class="bg-primary flex gap-1 items-center text-alt px-4 py-2 rounded-md ml-auto">
